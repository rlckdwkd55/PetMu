	...
	<%@ include file="/views/common/sideMenu.jsp" %>	
        ...
        <div class="boardArea">
            <div id="boardTitle" style="font-family: 'Jua', sans-serif; font-size : 30px; padding-bottom : 10px;">
                ...
                <label style="color : orange;"> 동반입장 장소 정보 공유</label>
            </div>
...
<script>
	...
	// 목록, 수정, 삭제 함수
	function goList(){
		location.href='<%= request.getContextPath()%>/selectList.bo?cate=5'; /* cate check */
	}
	
	function update(){
		location.href='<%= request.getContextPath()%>/updateView.bo?cate=5&bno=<%=b.getBno() %>'; /* cate check */
	}
	...
	function deleteOk(){
     	if (!confirm("삭제시 복구 할 수 없습니다. 삭제하시겠습니까?")) {
	        ...
	        // 확인(예) 버튼 클릭 시 이벤트
	    	location.href='<%= request.getContextPath()%>/delete.bo?cate=5&bno=<%=b.getBno() %>';   /* cate check */     
	    }
