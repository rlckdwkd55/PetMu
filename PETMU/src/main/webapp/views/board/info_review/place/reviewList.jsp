...
<body>
        ...
        <div class="boardArea">
    		...
    		<%if(pi.getListCount() > limit) {%>
	            <div class="pagingArea" align="center">
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=1&cate=6'"><<</button> <%--cate check --%>
	    			<%-- currentPage, stardPage, endPage, listCount받아와서 페이징 --%>
	    			<%if(currentPage <= 1) {%>
	    			<button disabled><</button>
	    			<%} else { %>
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=currentPage - 1%>&cate=6'"><</button>
	    			<%} %>
	    			...
	    			%>
	    				...
	    				<%} else { %>
	    				<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=p%>&cate=6'"><%=p%></button>
	    				<%} %>
	    			<%} %>
	    			...
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=currentPage + 1%>&cate=6'">></button>
	    			<%} %>
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=maxPage %>&cate=6'">>></button>
	            </div>
            <%} %>
    ...
    <script>
    	...
    	function post(){
	    	location.href = "<%= request.getContextPath() %>/insertView.bo?cate=6";
    	}
    	...
    	$('#listArea td').mouseenter(function(){
    		...
    		location.href = "<%= request.getContextPath() %>/selectOne.bo?cate=6&bno=" + bno; <%--cate check --%>
    	});
    	...
    	function search(){
    		...
    		if(!keyword){
    			...
    			location.href = "<%= request.getContextPath()%>/selectList.bo?cate=6&searchTarget=" + target + "&keyword=" + keyword; <%--cate check --%>
    		}
    	}
    	...
    	$('#sortList').on('change', function(){
    		...
    		case "new" : location.href = "<%= request.getContextPath()%>/selectList.bo?cate=6";
    					 break;
    		case "like" : location.href = "<%= request.getContextPath()%>/selectSortList.bo?cate=6&sort=" + sortL;
						   break;
    		case "cmt" : location.href = "<%= request.getContextPath()%>/selectSortList.bo?cate=6&sort=" + sortL;
			   			   break;
    		}
