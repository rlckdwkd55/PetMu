...
<body>
        ...
        <div class="boardArea">
    		...
    		<%if(pi.getListCount() > limit) {%>
	            <div class="pagingArea" align="center">
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=1&cate=4'"><<</button> <%--cate check --%>
	    			<%-- currentPage, stardPage, endPage, listCount받아와서 페이징 --%>
	    			<%if(currentPage <= 1) {%>
	    			<button disabled><</button>
	    			<%} else { %>
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=currentPage - 1%>&cate=4'"><</button>
	    			<%} %>
	    			...
	    			%>
	    				...
	    				<%} else { %>
	    				<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=p%>&cate=4'"><%=p%></button>
	    				<%} %>
	    			<%} %>
	    			...
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=currentPage + 1%>&cate=4'">></button>
	    			<%} %>
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=maxPage %>&cate=4'">>></button>
	            </div>
            <%} %>
    ...
    <script>
    	...
    	function post(){
	    	location.href = "<%= request.getContextPath() %>/insertView.bo?cate=4";
    	}
    	...
    	$('#listArea td').mouseenter(function(){
    		...
    		location.href = "<%= request.getContextPath() %>/selectOne.bo?cate=4&bno=" + bno; <%--cate check --%>
    	});
    	...
    	function search(){
    		...
    		if(!keyword){
    			...
    			location.href = "<%= request.getContextPath()%>/selectList.bo?cate=4&searchTarget=" + target + "&keyword=" + keyword; <%--cate check --%>
    		}
    	}
    	...
    	$('#sortList').on('change', function(){
    		...
    		case "new" : location.href = "<%= request.getContextPath()%>/selectList.bo?cate=4";
    					 break;
    		case "like" : location.href = "<%= request.getContextPath()%>/selectSortList.bo?cate=4&sort=" + sortL;
						   break;
    		case "cmt" : location.href = "<%= request.getContextPath()%>/selectSortList.bo?cate=4&sort=" + sortL;
			   			   break;
    		}
