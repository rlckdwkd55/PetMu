...
<script>
	...
	// 목록, 수정, 삭제 함수
	function goList(){
		location.href='<%= request.getContextPath()%>/selectList.bo?cate=7'; /* cate check */
	}
	
	function update(){
		location.href='<%= request.getContextPath()%>/updateView.bo?cate=7&bno=<%=b.getBno() %>'; /* cate check */
	}
	...
	function deleteOk(){
     	if (!confirm("삭제시 복구 할 수 없습니다. 삭제하시겠습니까?")) {
	        ...
	        // 확인(예) 버튼 클릭 시 이벤트
	    	location.href='<%= request.getContextPath()%>/delete.bo?cate=7&bno=<%=b.getBno() %>';   /* cate check */     
	    }
