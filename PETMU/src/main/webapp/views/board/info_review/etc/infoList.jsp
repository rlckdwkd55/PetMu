...
<body>
	...
	<%@ include file="/views/common/sideMenu.jsp" %>	
        ...
        <div class="boardArea">
            <div id="boardTitle" style="font-family: 'Jua', sans-serif; font-size : 30px; padding-bottom : 10px;">
                ...
                <label style="color : orange;"> 기타 장소 공유</label>
            </div>
    		...
    		<%if(pi.getListCount() > limit) {%>
	            <div class="pagingArea" align="center">
	    			...
	    			<%} else { %>
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=currentPage - 1%>&cate=7'"><</button>
	    			<%} %>
	    			...
	    			%>
	    				...
	    				<%} else { %>
	    				<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=p%>&cate=7'"><%=p%></button>
	    				<%} %>
	    			<%} %>
	    			...
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=currentPage + 1%>&cate=7'">></button>
	    			<%} %>
	    			<button onclick="location.href='<%= request.getContextPath() %>/selectList.bo?currentPage=<%=maxPage %>&cate=7'">>></button>
	            </div>
	   		<%}%>  	       
    ...
    <script>
    	...
    	function post(){
	    	location.href = "<%= request.getContextPath() %>/insertView.bo?cate=7";
    	}
    	...
    	$('#listArea td').mouseenter(function(){
    		...
    		location.href = "<%= request.getContextPath() %>/selectOne.bo?cate=7&bno=" + bno; <%--cate check --%>
    	});
    	...
    	function search(){
    		...
    		if(!keyword){
    			...
    			location.href = "<%= request.getContextPath()%>/selectList.bo?cate=7&searchTarget=" + target + "&keyword=" + keyword; <%--cate check --%>
    		}
    	}
    	...
    	$('#sortList').on('change', function(){
    		...
    		case "new" : location.href = "<%= request.getContextPath()%>/selectList.bo?cate=7";
    					 break;
    		case "like" : location.href = "<%= request.getContextPath()%>/selectSortList.bo?cate=7&sort=" + sortL;
						   break;
    		case "cmt" : location.href = "<%= request.getContextPath()%>/selectSortList.bo?cate=7&sort=" + sortL;
			   			   break;
    		}
