 ...
 */
@WebServlet("/selectSortList.bo")
public class BoardSortList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public BoardSortList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(target == null && keyword == null) {
			// 검색하지 않았을 때
			if(sort.equals("cmt")) {
				list = service.selectListSortcmt(cateNo, currentPage, limit);				
			} else if(sort.equals("like")) {
				list = service.selectListSortlike(cateNo, currentPage, limit);
			}
		} else if(target.equals("title") && keyword != null) {
			// 제목으로 검색
